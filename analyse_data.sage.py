# This file was *autogenerated* from the file /home/erick/Dropbox/erick/dotaproj/postpycon/analyse_data.sage
from sage.all_cmdline import *   # import sage library
_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_0p35 = RealNumber('0.35'); _sage_const_20 = Integer(20)
import csv
...
def build_game(row_player_file):
    ...
    f.close()
    bi_matrices.append(-_sage_const_1 *bi_matrices[_sage_const_0 ])
    return NormalFormGame(bi_matrices)

g=build_game("/home/erick/Dropbox/erick/dotaproj/postpycon/advantages_subset.csv")
m = g.payoff_matrices()[_sage_const_0 ].n(digits=_sage_const_2 )
sum_of_threats = [sum([row[k] for row in m])  for k in range(_sage_const_20 )]
...
def get_best_response_dictionary(game):
    ...
    for i in range(_sage_const_1 , N + _sage_const_1 ):
        strategy = [_sage_const_0  for k in range(i - _sage_const_1 )] + [_sage_const_1 ] + [_sage_const_0  for k in range(N - i)]
        hero = hero_names[i - _sage_const_1 ]
        best_responses = [hero_names[br] for br in g.best_responses(strategy,_sage_const_0 )]
...
def analyse_set_of_NE(NEs):
    ...
    return [sum([vector(ne[player])/len(NEs) for ne in NEs]) for player in [_sage_const_0 , _sage_const_1 ]]  
...
def plot_mean_NE(game, output_file, title=None):
    ...
    N = len(hero_names)
    p1 = mean_ne[_sage_const_0 ]
    p2 = mean_ne[_sage_const_1 ]


    heroes = range(N)  # the x locations for the heroes
    width = _sage_const_0p35        # the width of the bars
...
def heroes_that_are_played(mean_ne):
    return [hero_names[i] for i in range(len(hero_names)) if mean_ne[_sage_const_0 ][i] !=_sage_const_0 ]
